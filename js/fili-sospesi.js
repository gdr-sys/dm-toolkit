...
const StaleThreads = (() => {
  ...
  // La soglia e' salvata nelle impostazioni della campagna corrente (App.saveActiveCampaign),
  // cosi' e' personalizzabile per campagna invece che uguale per tutte, ed e' sincronizzata
  // insieme al resto dei dati (Firebase/Drive) senza bisogno di un canale a parte.
  const _getThreshold = () => {
    const camp = App.getActiveCampaign();
    const v = parseInt(camp?.staleThreadsDays, 10);
    return (v && v > 0) ? v : DEFAULT_THRESHOLD_DAYS;
  };

  const setThreshold = (val) => {
    const days = Math.max(1, Math.min(365, parseInt(val, 10) || DEFAULT_THRESHOLD_DAYS));
    App.saveActiveCampaign({ staleThreadsDays: days });
    render();
  };
  ...
  const _ensureCard = () => {
    ...
    card.innerHTML =
      '<div class="card-header" style="flex-wrap:wrap;gap:6px;">' +
        ...
        '<div style="display:flex;align-items:center;gap:5px;font-size:0.7rem;color:var(--text-muted);">' +
          '<span id="dash-stale-count"></span>' +
          '<span>non toccati da</span>' +
          '<input id="dash-stale-threshold" type="number" min="1" max="365" step="1" ' +
            'style="width:46px;padding:2px 4px;font-size:0.72rem;text-align:center;background:var(--bg-secondary);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text-primary);" ' +
            'onchange="StaleThreads.setThreshold(this.value)">' +
          '<span>giorni</span>' +
        '</div>' +
      '</div>' +
  ...
  const render = () => {
    ...
    const thrInput  = document.getElementById('dash-stale-threshold');
    if (!card || !list) return;
    card.style.display = '';

    const threshold = _getThreshold();
    if (thrInput && document.activeElement !== thrInput) thrInput.value = threshold;

    const items = _collect(camp, threshold);
    ...
    list.innerHTML = items.length
      ...
      : '<div class="text-muted text-sm" style="padding:6px 0;">Tutto aggiornato di recente.</div>';
  };
  ...
  return { render, openItem, setThreshold };
})();
